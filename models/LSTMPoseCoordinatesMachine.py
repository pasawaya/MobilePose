...
class PretrainCoordinateLPM(nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels, T=5):
        super(PretrainCoordinateLPM, self).__init__()
    ...
    def forward(self, x, centers):
        ...
        h_0, h_1 = dsntnn.flat_softmax(f_0), dsntnn.flat_softmax(b_1)
        p_0, p_1 = dsntnn.dsnt(h_0), dsntnn.dsnt(h_1)
        heatmaps = [h_0, h_1]
        coordinates = [p_0, p_1]
        ...
        for t in range(self.T - 1):
            ...
            h_t = dsntnn.flat_softmax(b_t)
            p_t = dsntnn.dsnt(h_t)
            heatmaps.append(h_t)
            coordinates.append(p_t)
